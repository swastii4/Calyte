...
export default function Home() {
  ...
  const games = [
    ...
    { title: "Mindful Maze", description: "Navigate through peaceful labyrinths", duration: "5-10 min" },
    { title: "Pattern Peace", description: "Create calming mandala patterns", duration: "15-30 min" }
  ];
      ...
      <div className="max-w-7xl mx-auto px-6 pt-12 pb-32">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 mb-20">
          {features.map((feature, index) => (
            <Link key={index} href={feature.link}>
              <Card className={`${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-900/5 border-slate-900/10'} backdrop-blur-md p-6 hover-elevate active-elevate-2 cursor-pointer transition-all h-full`}>
                <feature.icon className={`w-6 h-6 ${isDarkMode ? 'text-white/90' : 'text-slate-900/90'}`} />
                <h3 className={`${isDarkMode ? 'text-white/90' : 'text-slate-900'} text-xl mb-2 mt-4`}>{feature.title}</h3>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 mb-20">
          {games.map((game, index) => (
            <Link key={index} href="/games">
              <Card className={`${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-900/5 border-slate-900/10'} backdrop-blur-md p-6 hover-elevate active-elevate-2 cursor-pointer transition-all h-full`}>
                <Gamepad2 className={`w-6 h-6 ${isDarkMode ? 'text-white/90' : 'text-slate-900/90'}`} />
                <h3 className={`${isDarkMode ? 'text-white/90' : 'text-slate-900'} text-xl mb-2 mt-4`}>{game.title}</h3>
